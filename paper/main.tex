...
\textbf{Lemma 1 (Hedging Necessity):} Given agents with rational preferences operating under physical constraints in a multi-agent system, and assuming positive interference probability without defense ($\sigma(0) < 1$), positive interference losses ($L(1) > 0$), and defense-dominant returns at low hedging ($\rho(0) > \gamma$), any stable equilibrium necessarily exhibits strategic hedging behavior, where agents allocate non-trivial resources to defensive measures against potential interference from other agents (SM-A.1). (These assumptions are motivated by physical constraints but are modeling choices; see SM-B.2 for discussion.)
\label{lemma:hedging}
...
This asymmetry—internal states opaque, external actions observable—justifies external mediation. Coordination requires verifying intentions (internal alignment), not just compliance (external actions). Since both direct simulation (Theorem 1) and alternative verification mechanisms face fundamental limitations (see above), external mediators whose actions are observable to all parties become necessary.

\emph{Answer to Question 2:} Under the resource allocation model, peer systems cannot fully verify each other's internal states via direct real-time simulation. Theorem 1 shows that simulation of an equal-capability peer exceeds available resources; alternative verification mechanisms face specification, binding, and adversarial-incentive limitations. Internal states remain opaque to peers, though external actions remain observable. This negative answer raises the stakes for Question 3: if both direct avenues appear blocked, can external mediation provide a path forward?
...
\emph{C2* ensures participation.} Voluntary coordination requires each agent's share to exceed removal gains. For symmetric agents with oversight value $\beta_\Omega$ per capita: $\beta_\Omega \geq \beta_\ell/N$ where $\beta_\ell$ is removal gain. As $N$ increases, per-agent removal benefits fall (dilution effect). Under symmetry, the Shapley allocation yields equal shares $x_i = \beta_\Omega$~\citep{shapley.1953}. For the additive value function $v(S) = |S| \cdot \beta_\Omega$, this allocation lies in the \emph{core}: for any coalition $S$, $\sum_{i \in S} x_i = |S| \beta_\Omega = v(S)$, so no coalition can improve by deviating (SM-C.3). This ensures individual rationality and coalition stability under symmetric, additive surplus~\citep{bernheim.peleg.whinston.1987}. For non-additive value functions (e.g., with complementarities or substitutabilities between agents), the core may be empty and stronger conditions are required. Heterogeneous systems require future work.
...
\noindent\textbf{Gödel Incompleteness~\citep{godel.1931}:} No sufficiently expressive formal system can prove its own consistency. This provides a suggestive (though not formally equivalent) analogy for AI systems: complete verification of another system's properties through internal state inspection faces fundamental barriers.
...
This is the standard Aumann CE obedience constraint~\citep{aumann.1974,aumann.1987} applied to the specific payoff structure of our stage game.

\subsubsection*{C.2 Condition C1** (Perfect Public Equilibrium)}

For dynamic sustainability, we apply folk theorem results for repeated games with perfect public monitoring~\cite{abreu.1990,fudenberg.maskin.1986}. Using grim trigger strategies where defection triggers permanent reversion to mutual defection (punishment payoff $-\beta_\kappa$ per period), we compute present values explicitly.

\textbf{Present values under grim trigger.} Cooperation yields per-period payoff $\beta_\alpha$; permanent punishment yields $-\beta_\kappa$. Deviation gives one-period gain $\beta_D$ followed by permanent punishment:
\begin{align}
...
\textbf{Participation constraint (individual rationality):} Each agent requires $\phi_i \geq \beta_\ell/N$ where $\beta_\ell$ is the one-time removal gain. Equivalently, $\beta_\Omega \geq \beta_\ell/N$. As $N$ increases, the per-agent removal benefit $\beta_\ell / N$ decreases (dilution effect), easing participation.

\textbf{Limitation:} For non-additive value functions (complementarities or substitutabilities between agents), coalitional rationality may fail and the core may be empty. Heterogeneous systems require stronger conditions (future work).
...
\textbf{Conclusion:}
$$q(\text{M},\text{H})_{\text{external}} \approx 0 \quad \text{(very low; almost impossible to hide)}$$
...
These are \emph{internal states}, not external actions. Therefore, the simulation incompleteness theorem applies:
...
Human institutions can serve as external mediators for four reasons. First, \textbf{simulation incompleteness} prevents machines from fully simulating each other's internal states in real-time when $C_1 \approx C_2$, and alternative verification mechanisms face specification, binding, and adversarial-incentive limitations. Second, \textbf{external observability suffices for mediation}: the mediator need only provide a correlation device (focal point) and verify compliance with recommendations, not internal motivation. Third, \textbf{asymmetric opacity favors external arbiters}: machines verifying the mediator face $q(\text{M},\text{H}) \approx 0$ (human defection is easy to detect), the mediator verifying machines needs only to monitor external compliance rather than internal states, and machines verifying each other's compliance face $q(\text{M},\text{M})_{\text{external}} \to 0$ (feasible). Fourth, \textbf{weak capacity provides credibility}: humans cannot compute fast enough to exploit coordination (limited $C_H$), making them credible neutral arbiters. The mediator's \emph{weakness} (computational limitations) prevents strategic manipulation.
