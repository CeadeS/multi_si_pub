...
No funding was received for this work. Funding source: Not applicable. Grant number: Not applicable.
...
\textbf{Figure~\ref{fig:functional} | Functional structure of stability conditions.}
...
\textbf{Figure~\ref{fig:v_dynamic} | Optimal group size and stability volume progression.}
...
\textbf{Figure~\ref{fig:scaling} | Benefit structure dependency of optimal group size.}
...
\textbf{Figure~\ref{fig:summary} | Comprehensive analysis overview.}
...
Table~\ref{tab:symbols} summarizes the notation and parameter ranges used throughout the main text and supplementary analyses.
...
The reported stability volume (V$_{\text{dynamic}} \approx 94\%$, SM-D) assumes uniform priors over baseline parameter ranges in Table~\ref{tab:hard_soft_bounds}. We analyze robustness to range specification by varying each parameter's bounds by ±20\% and recomputing stability volumes.
...
\textbf{Results.} Table~\ref{tab:range_sensitivity} reports the stability-volume response to ±20\% range adjustments for each parameter.
