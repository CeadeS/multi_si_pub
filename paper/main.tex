...
\abstract{Geopolitical competition may lead to deployment of multiple advanced autonomous AI systems without centralized control. We analyze coordination stability using game-theoretic methods under physical constraints from thermodynamics, information theory, and relativity. Within a stylized model, we derive three stability conditions: one-shot obedience ($\beta_\alpha + \beta_\kappa \geq \beta_D$), dynamic sustainability ($\delta \geq \delta^*$), and participation ($\phi_i \geq \beta_\ell/N$), and characterize when strategic hedging emerges. A simulation incompleteness result indicates that peer systems may be unable to verify internal states via direct real-time simulation under equal resource budgets, motivating external mediation. Optimal group size depends on benefit structure (2--10 in our scenarios). Range sensitivity analysis identifies coordination quality ($\beta_\alpha$) as the critical uncertainty. We examine a "weak Leviathan" mechanism where human institutions could provide governance through observable weakness rather than superior capability. Four falsifiable predictions enable empirical adjudication. Results are conditional on parameter estimates; baseline simulations yield 94\% dynamic stability, with 78--95\% under coordination-quality range variation.}
...
Universal hedging creates inefficiencies—defensive resource locks, arms races, diverted innovation—but without coordination mechanisms, hedging is difficult to eliminate.
...
Theorem~1 suggests an asymmetry: machines face severe limits in monitoring each other's internal states ($q(\text{M},\text{M})_{\text{internal}} \to 1$), while monitoring humans can be easier ($q(\text{M},\text{H}) \approx 0$). This motivates a counterintuitive governance mechanism. Traditional Leviathan theory (Hobbes) requires a powerful central authority to enforce cooperation—superior capability to punish defectors. Our framework suggests the opposite may hold in this setting: governance can emerge through observable \emph{weakness} rather than strength.

\textbf{Theorem 3 (Weak Leviathan Feasibility):} A mediator $M$ can in principle sustain coordination among agents $\{A_1, \ldots, A_N\}$ if the following four conditions hold:
...
This formalizes \emph{governance through weakness}. The mediator need not be more capable—it may only provide a focal point~\cite{schelling.1960}. Asymmetric observability (Theorem~1) enables this: machines can monitor human mediators with high reliability ($q(\text{M}, \text{H}) \approx 0$), creating common knowledge of signals. The mediator's weakness—inability to enforce or punish—can make voluntary coordination more credible.

Human limitations may become advantages in this role. Biological processing ($\sim$kHz) is $10^6$ times slower than electronic circuits ($\sim$GHz), making humans comparatively non-threatening. Physical vulnerability creates dependence on functioning infrastructure, which may align incentives against catastrophic conflict. Inability to rapidly self-improve may also reduce commitment concerns. Humans can thereby serve as Schelling focal points—neutral through limited capability.

Two roles emerge, neither requiring superior capability. As \emph{coordination focal points}, observable human actions create common knowledge despite computational inferiority. As \emph{deterrence sources}, the possibility of imposing large costs could raise effective $\beta_\kappa$, reducing patience requirements via $\delta^* = g/(g+\beta_\kappa)$.
...
These conditions derive from established game theory but incorporate information-theoretic and relativistic bounds as fundamental parameters rather than idealized assumptions. The simulation incompleteness theorem provides a resource-accounting foundation for asymmetric observability, supporting why external mediation may be useful rather than merely convenient.

Several findings have policy-relevant implications. First, achieving the patience-free regime ($\beta_\alpha \geq \beta_D$) is a plausible design target. This eliminates dependence on uncertain discount factors—coordination becomes individually rational in the stage game. In the model, this requires high coordination signal quality through transparency requirements, cryptographic commitment protocols, and verifiable communication channels~\cite{verification.methods.2024,oxford.aigi.2025}. Second, optimal group size is not universal but depends on benefit structure—policymakers must first characterize whether benefits exhibit diminishing returns (saturating), constant returns (linear), or increasing returns (network effects) before selecting $N$. Third, range sensitivity analysis prioritizes empirical measurement: coordination quality ($\beta_\alpha$) matters most (sensitivity $-0.80$), while conflict cost and removal gain estimates are less influential (sensitivity $< 0.10$).

The temporal structure of deployment may create an intervention window. During Phase 1 (infrastructure dependence), when systems rely on human-controlled compute, energy, and communication substrates, conventional policy levers can shape parameters: proliferation controls bound $N$, architectural requirements increase $\beta_\alpha$ and decrease $\beta_D$, infrastructure dependencies raise $\beta_\Omega$ and increase exit costs. Once systems achieve autonomy (Phase 2), enforcement may become harder, and parameter values shaped during Phase 1 may influence whether incentives favor cooperation.

The weak Leviathan mechanism suggests humans could play two distinct roles post-autonomy. As coordination focal points, observable human actions may create common knowledge of mediator signals despite computational inferiority—an information-theoretic function rather than capability-based. As deterrence sources, capacity to impose large costs (even if mutually harmful) may raise effective conflict parameter $\beta_\kappa$, reducing patience requirements via the hyperbolic relationship $\delta^* = g/(g+\beta_\kappa)$ where $g = \beta_D - \beta_\alpha$. Neither role requires superior capability, only observable weakness combined with credible commitment.
...
The framework indicates that stable multi-agent coordination may be possible in principle, constrained by physical laws and game-theoretic structure. Quantitative predictions are conditional, but structural insights—simulation incompleteness motivates external mediation, the patience-free regime reduces temporal dependence, and optimal size varies with benefit structure—remain informative across parameter variations. Whether this possibility becomes reality depends on whether Phase 1 infrastructure control can shape parameters into stable regions.
