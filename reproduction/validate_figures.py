...
def _load_results() -> dict:
    ...
    for run_name in ("revision_experiments_jair",):
...
def validate() -> Dict[str, object]:
    ...
    # Under the corrected imperfect-monitoring model (q ~ U[0, 0.3]) the 95%
    # crossing moves to N=7 (it was N=4 in the legacy perfect-monitoring runs).
    cross_95 = next((n for n, v in zip(n_values, v_dynamic) if v >= 0.95), None)
    if cross_95 == 7:
        checks.append("V_dynamic crosses 95% at N=7 (corrected model, q sampled)")
    else:
        warnings.append(f"V_dynamic 95% crossing at N={cross_95}, expected N=7")
