...
def figure_sensitivity_analysis(path: str) -> None:
    ...
    delta_margin_q = np.array([delta_assumed - delta_crit(params, q=q) for q in q_vals])
    ax_q.plot(q_vals, c1_q, color="#64748b", linestyle="--", linewidth=1.4, label="C1* margin")
    ax_q.plot(
        q_vals,
        delta_margin_q,
        color="#1f5a99",
        linewidth=2,
        label=r"C1** margin ($\delta=0.9$ $-$ $\delta^*(q)$)",
    )
    ...
    _set_margin_ylim(ax_q, c1_q, delta_margin_q)
...
def figure_sobol_by_n(path: str, results_path: str) -> None:
    ...
    ax1.set_title(r'(a) Total-Order Sensitivity vs N ($\delta$ share grows with N)', fontsize=12)
    ax1.set_ylim([0, 1])
...
def figure_summary(path: str, results_path: str) -> None:
    ...
    def _top_driver(n_key: str) -> str:
        sdata = sobol_per_n[n_key]
        out_idx = sdata['output_names'].index('s_dynamic')
        st_col = [row[out_idx] for row in sdata['S_total']]
        best = int(np.argmax(st_col))
        return sdata['parameter_names'][best]

    driver_lines = []
    if sobol_per_n:
        n_keys = sorted(sobol_per_n.keys(), key=int)
        driver_lines = [
            f'• N={n_keys[0]}: {_top_driver(n_keys[0])} dominates',
            f'• N={n_keys[-1]}: {_top_driver(n_keys[-1])} dominates',
            '  (δ share grows with N)',
        ]
    cross_line = (
        f'• N={cross_95}: Crosses 95% threshold' if cross_95 is not None
        else '• 95% threshold not crossed'
    )
    takeaway_text = [
        'KEY FINDINGS:',
        '',
        f'• N<{cross_95}: Risky (<95% stable)' if cross_95 is not None else '• All N below 95%',
        cross_line,
        f'• N=10: {v_dyn[-1]:.1%} (saturating)',
        '',
        *driver_lines,
        '',
        '• δ* = 0 achievable',
        ...
        '   robust to any q)',
        '',
        f'• V_dynamic = {volume["V_dynamic"]*100:.1f}%',
        '  (uniform prior, q∈[0,0.3])',
    ]
