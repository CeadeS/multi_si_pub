...
def validate() -> Dict[str, object]:
    ...
    # Immediate-enforcement threshold delta*(q, s) at rho = 1 (Corollary 6.2):
    # residual gain g_s = max(0, beta_D - beta_alpha - (1-q)*beta_kappa),
    # delta*(q, s) = g_s / (g_s + (1-q)*kappa_eff), vanishing iff C1*(q) holds.
    imm = ModelParams(
        beta_kappa=0.5, beta_alpha=0.3, beta_D=1.2, beta_Omega=1.0, beta_ell=1.5, G0=1.0
    )
    q_imm = 0.2
    g_s = max(0.0, imm.beta_D - imm.beta_alpha - (1.0 - q_imm) * imm.beta_kappa)
    kappa_eff_imm = imm.beta_alpha + imm.beta_kappa
    delta_star_s = 0.0 if g_s <= 0.0 else g_s / (g_s + (1.0 - q_imm) * kappa_eff_imm)
    if _near(g_s, 0.5) and _near(delta_star_s, 0.5 / (0.5 + 0.8 * 0.8)):
        checks.append("Immediate threshold delta*(q,s) matches g_s/(g_s+(1-q)*kappa_eff) at q=0.2")
    else:
        warnings.append(f"Immediate threshold mismatch: g_s={g_s:.3f}, delta*_s={delta_star_s:.6f}")

    # Myopic-subset identification: g_s = 0 iff C1*(q) holds (both directions).
    myo = ModelParams(
        beta_kappa=1.0, beta_alpha=0.3, beta_D=1.0, beta_Omega=1.0, beta_ell=1.5, G0=1.0
    )
    g_s_myo = max(0.0, myo.beta_D - myo.beta_alpha - (1.0 - 0.3) * myo.beta_kappa)
    if (
        g_s_myo == 0.0
        and c1_margin(myo, q=0.3) >= 0.0
        and g_s > 0.0
        and c1_margin(imm, q=q_imm) < 0.0
    ):
        checks.append("g_s = 0 iff C1*(q) holds (myopic subset, both directions)")
    else:
        warnings.append("Myopic-subset identification g_s = 0 <-> C1*(q) failed")

    # qbar(delta) admissible-set cases: NaN when empty (delta below delta*(0)),
    # 1.0 when patience-free, and the round trip delta*(qbar(delta)) = delta on
    # the feasible interior.
    infeasible = ModelParams(
        beta_kappa=0.5, beta_alpha=0.3, beta_D=0.9, beta_Omega=1.0, beta_ell=1.5, G0=1.0
    )
    qb_empty = q_max_tolerable(infeasible, 0.1)  # delta*(0) = 0.6/1.4 ~ 0.429 > 0.1
    patience_free_params = ModelParams(
        beta_kappa=0.5, beta_alpha=0.9, beta_D=0.4, beta_Omega=1.0, beta_ell=1.5, G0=1.0
    )
    qb_free = q_max_tolerable(patience_free_params, 0.5)
    qb_interior = q_max_tolerable(infeasible, 0.6)  # expect 0.5
    roundtrip = delta_crit(infeasible, q=qb_interior)
    if math.isnan(qb_empty) and qb_free == 1.0 and _near(roundtrip, 0.6, tol=1e-9):
        checks.append("qbar cases: NaN on empty set, 1.0 patience-free, delta*(qbar)=delta interior")
    else:
        warnings.append(
            f"qbar cases failed: empty={qb_empty}, free={qb_free}, roundtrip={roundtrip}"
        )
    ...
    if _near(n_star, 5.5):
        checks.append("Group-value optimum N* = f/c + 1/2 (exact pairwise costs)")
    else:
        warnings.append(f"Group-value optimum mismatch: {n_star:.3f}")
