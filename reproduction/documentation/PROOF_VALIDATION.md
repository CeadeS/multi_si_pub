...
- ✓ Immediate threshold delta*(q,s) matches g_s/(g_s+(1-q)*kappa_eff) at q=0.2
- ✓ g_s = 0 iff C1*(q) holds (myopic subset, both directions)
- ✓ qbar cases: NaN on empty set, 1.0 patience-free, delta*(qbar)=delta interior
...
- ✓ Group-value optimum N* = f/c + 1/2 (exact pairwise costs)
